...
export class OrbitalTextureRenderer {
    ...
    async _initializeWebGPU() {
        ...
        await this._createWebGPUCompositePipeline();
        await this._createWebGPUNormalPipeline();

        console.log('[OrbitalTextureRenderer] WebGPU pipelines initialized');
    }
    ...
    async computeTileAverageColors() {
        ...
        if (this._apiName === 'webgpu') {
            return await this._computeTileAverageColorsWebGPU(atlasTexture, maxTileTypes, numSeasons);
        } else {
            return this._createFallbackAverageColors(maxTileTypes, numSeasons);
        }
    }

    async _computeTileAverageColorsWebGPU(atlasTexture, maxTileTypes, numSeasons) {
    ...
    async _createWebGPUCompositePipeline() {
    ...
    async _createWebGPUNormalPipeline() {
    ...
    async renderFaceTextures(face, tileDataTexture, heightDataTexture) {
        ...
        if (this._apiName === 'webgpu') {
            colorTexture = await this._compositeColorTextureWebGPU(face, tileDataTexture);
            normalTexture = await this._generateNormalMapWebGPU(face, heightDataTexture);
        } else {
            colorTexture = await this._compositeColorTextureWebGL2(face, tileDataTexture);
            normalTexture = await this._generateNormalMapWebGL2(face, heightDataTexture);
        }
    ...
    async _compositeColorTextureWebGPU(face, tileDataTexture) {
        const size = this.outputResolution;
    ...
    async _generateNormalMapWebGPU(face, heightDataTexture) {
        const size = this.outputResolution;
    ...
    async _initializeWebGL2() {
        console.log('[OrbitalTextureRenderer] WebGL2 mode - using CPU fallbacks');
    }

    async _compositeColorTextureWebGL2(face, tileDataTexture) {
        return this._createFallbackColorTexture(face, tileDataTexture);
    }

    async _generateNormalMapWebGL2(face, heightDataTexture) {
        return this._createFlatNormalMap(this.outputResolution);
    }
