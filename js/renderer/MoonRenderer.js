...
export class MoonRenderer {
    constructor(backend) {
        ...
        this.direction = new THREE.Vector3(0, 0.5, 0.5).normalize();
        ...
        this.color = new THREE.Color(0.9, 0.92, 0.98); // Slightly cool white
        ...
        this._apiName = 'webgl2';
    }

    async initialize() {
        if (this._initialized) return;
        
        this._apiName = this.backend.getAPIName?.() || 'webgl2';
    ...
    async _initializeWebGL2() {
        const vertexShader = this._getMoonVertexGLSL();
        const fragmentShader = this._getMoonFragmentGLSL();
        
        this._material = new Material({
            name: 'MoonRenderer_WebGL2',
            vertexShader,
            fragmentShader,
            uniforms: {
                moonDirection: { value: new THREE.Vector3() },
                moonAngularDiameter: { value: 0.009 },
                moonPhase: { value: 0.5 },
                moonIllumination: { value: 1.0 },
                moonColor: { value: new THREE.Color(0.9, 0.92, 0.98) },
    ...
    update(camera, moonInfo, sunDirection) {
        ...
        this._sunDirection = sunDirection ? sunDirection.clone().normalize() : new THREE.Vector3(0, 1, 0);
    ...
    render(camera) {
        ...
        if (this._apiName === 'webgpu') {
            this._renderWebGPU(camera);
        } else {
            this._renderWebGL2(camera);
        }
    }
    _renderWebGPU(camera) {
    ...
    _renderWebGL2(camera) {
        const uniforms = this._material.uniforms;
        
        uniforms.moonDirection.value.copy(this.direction);
        uniforms.moonAngularDiameter.value = this.angularDiameter;
        uniforms.moonPhase.value = this.phase;
        uniforms.moonIllumination.value = this.illumination;
        uniforms.moonColor.value.copy(this.color);
        uniforms.moonIntensity.value = this.intensity * this.opacity;
        uniforms.sunDirection.value.copy(this._sunDirection || new THREE.Vector3(0, 1, 0));
...
void main() {
    ...
    gl_Position = vec4(pos, 0.9998, 1.0);
    vUv = pos * 0.5 + 0.5;
}
`;
    }

    _getMoonFragmentGLSL() {
        return `#version 300 es
precision highp float;
...
uniform float moonAngularDiameter;
...
float getPhaseShading(vec2 localUV, float phase) {
    float terminatorX = cos(phase * 2.0 * PI);
...
void main() {
    ...
    float moonRadius = moonAngularDiameter * 0.5;
    ...
    float edgeSoftness = moonRadius * 0.1;
    float discMask = smoothstep(cosMoonRadius - edgeSoftness, cosMoonRadius + edgeSoftness * 0.3, cosAngle);
    
    float angle = acos(clamp(cosAngle, -1.0, 1.0));
    float distFromCenter = angle / moonRadius;
    
    vec3 toPixel = rayDir - moonDir * cosAngle;
    float toPixelLen = length(toPixel);
    vec2 localUV = vec2(0.0);
    
    if (toPixelLen > 0.0001) {
        vec3 toPixelNorm = toPixel / toPixelLen;
        vec3 moonUp = vec3(0.0, 1.0, 0.0);
        vec3 moonRight = normalize(cross(moonUp, moonDir));
        vec3 moonActualUp = normalize(cross(moonDir, moonRight));
        
        localUV.x = dot(toPixelNorm, moonRight) * distFromCenter;
        localUV.y = dot(toPixelNorm, moonActualUp) * distFromCenter;
    }
    
    float phaseShading = getPhaseShading(localUV, moonPhase);
    float limbDarkening = 1.0 - pow(distFromCenter, 2.0) * 0.2;
    
    vec3 baseColor = moonColor * moonIntensity * moonIllumination;
    vec3 shadedColor = baseColor * phaseShading * limbDarkening;
    
    float glowAmount = smoothstep(1.0, 0.7, distFromCenter) * phaseShading * 0.1;
    vec3 finalColor = shadedColor + moonColor * glowAmount;
    
    float alpha = discMask;
