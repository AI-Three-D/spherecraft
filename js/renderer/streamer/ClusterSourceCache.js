...
export class ClusterSourceCache {
    ...
    initialize(tileCache) {
        ...
        this._instanceBuffer = this.device.createBuffer({
            label: 'ClusterSourceCache-Instances',
            size: Math.max(256, this._tilePoolSize * this._cfg.perLayerCapacity * CLUSTER_INSTANCE_STRIDE),
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
        });
        this._counterBuffer = this.device.createBuffer({
            label: 'ClusterSourceCache-Counts',
        ...
        const memMB = (this._tilePoolSize * this._cfg.perLayerCapacity * CLUSTER_INSTANCE_STRIDE) / (1024 * 1024);
        Logger.info(
...
export const CLUSTER_SOURCE_FLAGS = Object.freeze({
    ACTIVE: ACTIVE_CLUSTER_FLAG,
    LAYER_META_U32_STRIDE,
    INSTANCE_STRIDE: CLUSTER_INSTANCE_STRIDE,
});
