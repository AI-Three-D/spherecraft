...
export class CelestialBody {
  ...
  /**
     ...
     * @returns {Object} {direction: THREE.Vector3, distance: number, intensity: number}
     */
  getStarDirection(star, localSurfacePos = { x: 0, y: 0, z: 0 }) {
    ...
    let systemDir = new THREE.Vector3(
        dx / distance,
        dy / distance,
        dz / distance
    );
    ...
    getLocalUp(localPos) {
        ...
        return new THREE.Vector3(localPos.x / len, localPos.y / len, localPos.z / len);
    }
     ...
     * @returns {THREE.Vector3} Position relative to camera (safe for GPU)
     */
