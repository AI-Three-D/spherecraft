...
fn sampleHeightNearestChunkLocal(localUV: vec2<f32>, atlasOffset: vec2<f32>, atlasScale: f32, lod: i32, layer: i32) -> f32 {
    ...
    let sampleUV = remapToTexelGrid(localUV, lod);
    ...
    let localCoord = clamp(sampleUV, vec2<f32>(0.0), vec2<f32>(1.0)) * vec2<f32>(maxLocalI);
...
fn sampleHeightChunkLocal(localUV: vec2<f32>, atlasOffset: vec2<f32>, atlasScale: f32, lod: i32, layer: i32) -> f32 {
    let texSize = vec2<f32>(textureDimensions(heightTexture));
    let globalMax = vec2<i32>(textureDimensions(heightTexture)) - vec2<i32>(1);
    var baseOffset = vec2<f32>(0.0);
    var baseScale = 1.0;
...
fn sampleHeightNearest(localUV: vec2<f32>, atlasOffset: vec2<f32>, atlasScale: f32, lod: i32, layer: i32) -> f32 {
    if (DEBUG_SAMPLE_FIX) {
        return sampleHeightNearestChunkLocal(localUV, atlasOffset, atlasScale, lod, layer);
    }
    let texSize = vec2<f32>(textureDimensions(heightTexture));
...
fn sampleHeight(localUV: vec2<f32>, atlasOffset: vec2<f32>, atlasScale: f32, sampleLOD: i32, _selfLOD: i32, layer: i32) -> f32 {
    if (DEBUG_SAMPLE_FIX) {
        return sampleHeightChunkLocal(localUV, atlasOffset, atlasScale, sampleLOD, layer);
    }
    let texSize = vec2<f32>(textureDimensions(heightTexture));
    let maxIdx = max(texSize - vec2<f32>(1.0), vec2<f32>(1.0));
