...
function encodeCoord(faceBits, value) {
    const encodedValue = faceBits === FLAT_FACE ? value + COORD_BIAS : value;
    return Number.isFinite(encodedValue) ? clampInt(encodedValue, 0, COORD_MAX) : 0;
}
...
export function mortonEncode(x, y) {
    let result = 0n;
    const ix = Number.isFinite(x) ? clampInt(x, 0, COORD_MAX) : 0;
    const iy = Number.isFinite(y) ? clampInt(y, 0, COORD_MAX) : 0;
...
export function createChunkKey(face, level, x, y) {
    const faceBits = face === null || face === undefined || face < 0 ? FLAT_FACE : clampInt(face, 0, 7);
    const lod = clampInt(level, 0, 31);
