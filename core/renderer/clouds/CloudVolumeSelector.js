...
import { Vector3, clamp } from '../../../shared/math/index.js';
...
export class CloudVolumeSelector {
    constructor(config = {}) {
        ...
        this._scratchCamSurface = new Vector3();
        this._scratchHoriz = new Vector3();
        ...
        this._debugAnchors = [
            { worldPos: new Vector3(), radiusH: 0, radiusV: 0, fogType: 0, coverage: 1.0, densityMult: 1.0 },
            { worldPos: new Vector3(), radiusH: 0, radiusV: 0, fogType: 2, coverage: 1.0, densityMult: 1.0 }
        ];
    ...
    _applyAutoScale(planetConfig) {
        ...
        const baseCell = clamp(atmoHeight * 0.12, 1200, 10000);
        ...
        if (this._tierAAuto) {
            this._tierAMaxDist = clamp(atmoHeight * 0.3, baseCell * 2.0, atmoHeight * 0.7);
        }
        if (this._tierBAuto) {
            this._tierBMaxDist = clamp(atmoHeight * 0.95, this._tierAMaxDist * 2.5, atmoHeight * 1.8);
        }
