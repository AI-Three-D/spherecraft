...
/**
 * Write a markdown instruction file consumable by an AI agent to apply fixes.
 * Stored as a GitHub Actions artifact so agents can fetch it via the API.
 */
function writeInstructionFile(summary, inlineComments, orphaned, model) {
  const severityOf = (body) => {
    if (/🔴/.test(body)) return "Critical";
    if (/🟡/.test(body)) return "Warning";
    return "Info";
  };

  const allComments = [...inlineComments, ...orphaned];
  const lines = [
    "# AI Code Review Fix Instructions",
    `<!-- Generated by ${model}. Feed this file to an AI agent to apply the suggested fixes. -->`,
    "",
    "## Summary",
    summary,
    "",
  ...
  if (allComments.length === 0) {
    lines.push("No specific line-level issues found.");
  } else {
    for (const c of allComments) {
      lines.push(`### \`${c.path}\` line ${c.line} — ${severityOf(c.body)}`);
      lines.push("");
      lines.push(c.body);
      lines.push("");
    }
  ...
  mkdirSync("ai-review-output", { recursive: true });
  writeFileSync("ai-review-output/review-instructions.md", lines.join("\n"), "utf8");
  console.log("Instruction file written to ai-review-output/review-instructions.md");
}
...
async function main() {
  ...
  writeInstructionFile(summary, inlineComments, orphaned, model);
